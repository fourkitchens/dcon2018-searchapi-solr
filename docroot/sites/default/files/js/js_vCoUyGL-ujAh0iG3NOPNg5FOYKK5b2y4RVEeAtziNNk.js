...
(function ($, Drupal, drupalSettings) {
      ...
      $('.paragraph-type-add-modal-button', context)
        ...
        .on('click', function (event) {
          ...
          if ($button.attr('name') === 'first_button_add_modal') {
            ...
            config.delta = $button.closest('tr').index() / 2;
          }
   ...
   * @type {Object}
   */
  Drupal.behaviors.initInBetweenButtons = {
    attach: function () {
      var $tables = $('.paragraphs-add-in-between-button-template-container')
        .parent()
        .siblings('table')
      ...
      $tables.each(function (index, table) {
        ...
        $table.find('> tbody > tr')
          .each(Drupal.modalAddParagraphs.inBetweenButtons.addButton);

        // Trigger attaching of behaviours for added buttons.
        Drupal.behaviors.modalAdd.attach($table);
      });

      for (var tableId in drupalSettings.tableDrag) {
        if (drupalSettings.tableDrag.hasOwnProperty(tableId)) {
          Drupal.modalAddParagraphs.inBetweenButtons.adjustDragDrop(tableId);

          jQuery('#' + tableId).once('in-between-buttons-columnschange').on('columnschange', function () {
            Drupal.modalAddParagraphs.inBetweenButtons.adjustDragDrop(tableId);
          });
        }
      }
   ...
   * @type {Object}
   */
  Drupal.modalAddParagraphs.inBetweenButtons = {};
   ...
   * @param rowElement
   */
  Drupal.modalAddParagraphs.inBetweenButtons.addButton = function (index, rowElement) {
    var $context = $(rowElement).closest('table').closest('div');

    // Get "add in between" button template and apply data on it.
    var $addButtonTemplate = $('.paragraphs-add-in-between-button-template', $context);
    var $buttonRow = _templateClone($addButtonTemplate, {});
   ...
   * @param tableId
   */
  Drupal.modalAddParagraphs.inBetweenButtons.adjustDragDrop = function (tableId) {
